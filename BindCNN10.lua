...
local addimgui = imgui.ImBool(false)
...
function main()
    ...
    sampAddChatMessage("[BindCNN10] Tools for CNN", main_color)

    sampRegisterChatCommand("bmenu", cmd_bmenu)
    sampRegisterChatCommand("bbmenu", cmd_bbmenu)
...
function cmd_bbmenu(arg)
    addimgui.v = not addimgui.v
    imgui.Process = addimgui.v
end
...
function cmd_vig(arg)
    ...
    lua_thread.create(function ()
        ...
        if not women.v == false then
            sampSendChat("/me взяла из кармана планшет и зашла в базу данных сотрудников...")
            wait(2000)
            sampSendChat("/me ...после чего изменила информацию о сотруднике, вписала 'выговор'")
            wait(2000)
            sampShowDialog(1000, "Система выдачи выговоров", "Введите id игрока и причину\n19,Нарушение устава", "Выдать",'Отмена', 1)
            while sampIsDialogActive(6406) do wait(100) end
            local result, button, list, input = sampHasDialogRespond(6406)
...
function cmd_exp(arg)
    ...
    lua_thread.create(function ()
        ...
        if not women.v == false then
            sampSendChat("/me взяла человека за шкирку и повела ко входу")
            wait(2000)
            sampSendChat("/todo В следующий раз будете вести себя лучше*выкидывая человека из радиоцентра")
            wait(2000)
            sampShowDialog(1000, "Система выпровода игрока", "Введите id игрока и причину\n19,Нарушение порядка", "Выдгнать",'Отмена', 1)
...
function cmd_fmt(arg)
	...
	lua_thread.create(function ()
        ...
        if not women.v == false then
            sampSendChat("/me взяла из кармана планшет и зашла в базу данных сотрудников...")
            wait(2000)
            sampSendChat("/me ...после чего заблокировала доступ к рации у сотрудника")
            wait(2000)
            sampShowDialog(1000, "Система выдачи мутов", "Введите id игрока и причину\n19,Неадекват", "Выдать",'Отмена', 1)
            while sampIsDialogActive(6406) do wait(100) end
            local result, button, list, input = sampHasDialogRespond(6406)
...
function cmd_unfmt(arg)
	...
	lua_thread.create(function ()
        ...
        if not women.v == false then
            sampSendChat("/me взяла из кармана планшет и зашла в базу данных сотрудников...")
            wait(2000)
            sampSendChat("/me ...после чего разблокировала доступ к рации у сотрудника")
            wait(2000)
            sampShowDialog(1000, "Система выдачи мутов", "Введите id игрока и причину\n19,Неадекват", "Выдать",'Отмена', 1)
            while sampIsDialogActive(6406) do wait(100) end
            local result, button, list, input = sampHasDialogRespond(6406)
...
function imgui.OnDrawFrame()
    if not main_window_state.v and not addimgui.v then
        imgui.Process =  false
    end
        ...
        imgui.Begin(' ', main_window_state, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse)
            ...
            imgui.BeginChild('##main', imgui.ImVec2(700, 400), true)
                ...
                elseif tab.v == 2 then -- Настройки
                    ...
                    imgui.SameLine()
                    imgui.TextQuestion("( ? )",u8"При активации отыгровки будут женскими. По стандарту они мужские.")
                    if imgui.Checkbox(u8"Включить уведомления", yved) then
                        mainIni.config.yvedoml = yved.v
                        inicfg.save(mainIni, directIni)
                    end 
                    imgui.SameLine()
                    imgui.TextQuestion("( ? )",u8"При активации у Вас будут высвечиваться уведомления")
    ...
    if addimgui.v then
        imgui.Begin(u8"Тест", addimgui)
        imgui.Text(u8(text_dialog))
        imgui.End()
    end
end
...
function sampev.onServerMessage(color, text)
	...
	if text:find("На обрабоку объявлений пришло VIP сообщение от:") then
        printStyledString("NEW VIP ADD", 500, 2) 
    end
end
...
function sampev.onShowDialog(id, style, t, btn, btn2, text)
    for line in text:gmatch("[^\n]+") do
        ...
        elseif line:find('Сообщение:%s*{......}%s*.+') then
            local add = line:match('Сообщение:%s*{......}%s*(.+)')
        end
        text_dialog = idplay.." - "..add
    end
end
