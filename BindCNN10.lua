...
function cmd_exp(arg)
    lua_thread.create(function ()
        sampSendChat("/me вз€л человека напротив за шкирку и потащил ко входу")
        wait(2000)
        sampSendChat("/todo ¬ следующий раз будете вести себ€ лучше!*выкидыва€ человека..")
        wait(2000)
        sampSendChat("/me ...и закрыва€ дверь в радиоцентр")
		wait(2000)
		sampShowDialog(1000, "—истема выдачи выговоров", "¬ведите id игрока и причину\n19,Ќарушение пор€дка", "¬ыдать",'ќтмена', 1)
		while sampIsDialogActive(6406) do wait(100) end
        local result, button, list, input = sampHasDialogRespond(6406)
        if input:find('(%d+),(.+)') and button == 1 then
            idn, itext = input:match('(%d+),(.+)')
