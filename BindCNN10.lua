...
function main()
    ...
    sampAddChatMessage("[BindCNN10] Ѕиндер дл€ CNN", main_color)
	sampAddChatMessage("[BindCNN10] јвтор: Kevin McWood", main_color)
	sampAddChatMessage("[BindCNN10] —пециально дл€ сервера Brainburg", main_color)
...
function cmd_exp(arg)
    ...
    local _, idpl = sampGetPlayerIdByCharHandle(ped)
    lua_thread.create(function ()
        sampSendChat("/me вз€л человека за шкирку и повел ко входу")
        wait(2000)
        sampSendChat("/todo ¬ следующий раз будете вести себ€ лучше*выкидыва€ человека из радиоцентра")
        wait(2000)
        sampShowDialog(1000, "—истема выпровода игрока", "¬ведите id игрока и причину\n19,Ќарушение пор€дка", "¬ыдгнать",'ќтмена', 1)
        while sampIsDialogActive(6406) do wait(100) end
        local result, button, list, input = sampHasDialogRespond(6406)
        if input:find('(%d+),(.+)') and button == 1 then
            idn, itext = input:match('(%d+),(.+)')
...
function cmd_fmt(arg)
	...
	wait(2000)
	sampShowDialog(1000, "—истема выдачи мутов", "¬ведите id игрока и причину\n19,Ќеадекват", "¬ыдать",'ќтмена', 1)
	while sampIsDialogActive(6406) do wait(100) end
	local result, button, list, input = sampHasDialogRespond(6406)
	if input:find('(%d+),(.+)') and button == 1 then
		idn, itext = input:match('(%d+),(.+)')
...
function imgui.OnDrawFrame()
	...
	imgui.TextColored(imgui.ImVec4(0, 1, 0, 1), u8'¬ерси€: ' .. updateIni.info.vers_text)
        -- ќсновное
    imgui.BeginChild('##ќсновное', imgui.ImVec2(200, 175), true)
    ...
    imgui.BeginChild('##“емы', imgui.ImVec2(200, 175), true)
    ...
    imgui.BeginChild('## оманды', imgui.ImVec2(200, 175), true)
    imgui.Text(u8"/bmenu - меню скрипта\n/invv - прин€тие игрока\n/clearchat - очистить чат\n/vig - выдать выговор\n/unvig - сн€ть выговор( в разр)\n/exp - выгнать человека\n/fmt - выдать мут\n/unfmt - сн€ть мут")
    imgui.EndChild()
		-- ѕоказ чего либо
	imgui.SetCursorPos(imgui.ImVec2(415, 220))
    imgui.BeginChild('##ѕоказ', imgui.ImVec2(200, 175), true)
	...
	if imgui.Button(u8'Ћицензии', imgui.ImVec2(150, 30)) then
		sampSendChat("/showlic " ..id)
	end
    imgui.EndChild()
        -- Ћекции
    imgui.SetCursorPos(imgui.ImVec2(415, 43))
    imgui.BeginChild('##Ћекции', imgui.ImVec2(200, 175), true)
	...
	imgui.BeginChild('##Ёфиры', imgui.ImVec2(200, 175), true)
	if imgui.Button(u8'/gov - —ћ» Ћ¬', imgui.ImVec2(150, 30)) then
		lua_thread.create(function ()
		sampSendChat("/gov [CNN LV] ”важаемые жители штата, прошу минуточку внимани€..")
		wait(6000)
		sampSendChat("/gov [CNN LV] ¬ холле –адиостанции г.Ћас-¬ентурас проходит собеседование.")
		wait(6000)
		sampSendChat("/gov [CNN LV] ∆дЄм всех желающих. ѕри себе иметь: паспорт, мед.карту, пакет лицензий.")
		wait(6000)
		end)
	end
