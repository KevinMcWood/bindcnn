...
function sampev.onShowDialog(id, style, t, btn, btn2, text)
    for line in text:gmatch("[^\n]+") do  -- разбиваешь на строки
        if line:find("Объявление от {......}(%w+_%w+), спустя (%d+)с") then -- если найден текст то
            local text = line:match("Объявление от {......}(%w+_%w+), спустя(%d+)с") -- парсишь ник и текст жб
...
--function sampev.onShowDialog(dialogId,s,t,b1,b2,text)
  --  for line in text:gmatch("[^\n]+") do -- разбиваем чтобы искать по строкам
       -- if line:find('Должность: %{......%}%[$(%d+)%]') then -- Поиск текста в строке. |  Пример строки Деньги в банке: [$21]
       --     money = line:match('Должность: %{......%}%[$(%d+)%]') -- запишет в переменную money цифру 21
